...
def apply(image, filter):
    ...
    out = numpy.clip(image, 0, 1)
    return out

def unsharp(image, gauss):
    blur = apply(image, gauss)
    mask = image - blur
    out = numpy.clip(image + mask, 0, 1)
    return out
...
sharp = unsharp(img, filter0)
collage = numpy.concatenate([img, sharp], axis=1)
